...
//
// It is used to generate the models for the pegwings-go library.
package main
...
func run(ctx context.Context) error {
	...
	req, err := http.NewRequestWithContext(
		ctx,
		"GET",
		"https://api.pegwings.ai/v1/models",
		nil,
	)
	if err != nil {
		return err
	}
	key := os.Getenv("PEGWINGS_KEY")
	if key == "" {
		return fmt.Errorf("PEGWINGS_KEY is not set")
	}
